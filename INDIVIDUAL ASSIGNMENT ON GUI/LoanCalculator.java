...
public class LoanCalculator implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
	        try {
	            ...
	            int years = Integer.parseInt(durationField.getText().replaceAll(" ", ""));
	            double total = amount + (amount * INTEREST_RATE * years);
	            // Format with spaces between every three digits
	            totalField.setText(String.format("%,.0f", total).replace(',', ' '));
	        } catch (Exception ex) {
